...
KeyringInstance createTestKeyring([KeyringOptions options, bool isDerived = true]) {
  ...
  PAIRS.forEach((p) {
    final name = p["name"];
    final publicKey = p["publicKey"] as Uint8List;
    final secretKey = p["secretKey"] as Uint8List;
    final seed = p["seed"] as String;
    final type = p["type"] ?? "sr25519";

    final meta = {"isTesting": true, "name": name ?? seed.replaceAll('//', '_').toLowerCase()};

    final pair = (!isDerived && name == null)
        ? keyring.addFromUri(seed, meta, options.type)
        : keyring.addPair(createPair(Setup(toSS58: keyring.encodeAddress, type: type),
            PairInfo(publicKey: publicKey, secretKey: secretKey), meta));
...
Map<String, KeyringPair> createTestPairs([KeyringOptions options, bool isDerived = true]) {
  final keyring = createTestKeyring(options, isDerived);
  final pairs = keyring.getPairs();
  final map = {"nobody": nobody()};

  return pairs.fold(map, (result, pair) {
    final name = pair.meta["name"];
