...
// const SECP256K1_BIP32_NAME: &str = "secp256k1 seed";
...
impl ExtendedSecretKey {
    ...
    pub fn derive_child(&self, index: ChildIndex) -> Result<Self> {
        ...
        mac.input(&[0u8]);
        mac.input(self.secret_key.to_bytes().as_ref());
        mac.input(index.to_bits().to_be_bytes().as_ref());
    ...
    #[inline]
    #[no_mangle]
    pub fn public_key_secp256k1(&self) -> Vec<u8> {
        let s = Secp256k1::new();
        let sbytes: [u8; 32] = self.secret_key.to_bytes();
        let sk = secp256k1::key::SecretKey::from_slice(&sbytes).unwrap();
...
fn hex_str(string: &str) -> Vec<u8> {
    let message_result: Result<Vec<u8>, FromHexError> = string.from_hex();
...
#[no_mangle]
pub fn root(seed: &str) -> ExtendedSecretKey {
    ExtendedSecretKey::from_seed(&hex_str(seed)).unwrap()
}

// #[no_mangle]
// pub extern "C" fn bip32_ed25519_keyPair_from_path(seed: &str, path: &str) {
//     let vector1_path: DerivationPath = path.parse().unwrap();
//     let node = root(seed).derive(&vector1_path).unwrap();
// }
...
mod tests {
    ...
    fn derivation_path() {
        ...
        let pk256 = node.public_key_secp256k1();
        assert_eq!(
            &pk256[..],
            &hex_str("0215197801d5ba7001d143183d04bf4e675be6c24b5101fc89de1659d50dbbaa24")[..]
        );
